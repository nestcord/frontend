...
import { Heart, MessageCircle, Share2, ImageIcon, MoreHorizontal } from 'lucide-react'
    ...
    return (
        ...
        <CardHeader className="flex items-start space-x-4 pb-2">
            ...
            <HoverCard>
              <HoverCardTrigger asChild>
                <div className="flex items-center space-x-2">
                  <Link href={`/app/user/${post.user.user_name}`} className="font-semibold hover:underline">
                    {post.user.name}
                  </Link>
                  <span className="text-neutral-400">@{post.user.user_name}</span>
              ...
              <HoverCardContent className="w-80 bg-neutral-900 border-neutral-700 text-white">
                ...
                <div className="space-y-1 mt-4">
                  ...
                  <p className="text-sm text-neutral-400">@{post.user.user_name}</p>
                </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 bg-neutral-900 border-neutral-700 text-white">
              <DropdownMenuItem className="text-red-500 focus:text-red-500 focus:bg-red-500/10">Delete post</DropdownMenuItem>
        ...
        <CardFooter className="flex items-center justify-between border-t border-neutral-800 pt-4">
          <div className="flex items-center space-x-4 text-sm">
            <Button variant="ghost" size="sm" className="text-neutral-400 hover:text-blue-400 hover:bg-blue-400/10">
              <Heart className="w-4 h-4 mr-2" />
              <span>{post.likes}</span>
            </Button>
            <Button variant="ghost" size="sm" className="text-neutral-400 hover:text-green-400 hover:bg-green-400/10">
              <MessageCircle className="w-4 h-4 mr-2" />
            </Button>
            <Button variant="ghost" size="sm" className="text-neutral-400 hover:text-purple-400 hover:bg-purple-400/10">
